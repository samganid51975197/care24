...
export async function GET(req:Request){return withActor(req,async actor=>{const db=getClient();const summary=await paymentSummary(db);const entries=actor.role==='admin'?(await db.execute('SELECT id,reference,kind,amount,payment_id,paid_on FROM care_payments ORDER BY created_at DESC')).rows:undefined;return privateJson({...summary,admin:actor.role==='admin',entries});})}
