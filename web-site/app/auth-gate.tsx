...
export default function AuthGate({children}:{children:React.ReactNode}) {
 ...
 return <><div className="account-bar"><span>{user.name} · {({admin:'관리자',hospital:'병원 담당자',caregiver:'간병인'} as Record<string,string>)[user.role]}</span><nav>{user.role==='admin'&&<a href="/admin">가입 승인·소속 관리</a>}<a href="/">업무 화면</a><button onClick={logout}>로그아웃</button></nav></div>{error&&<p role="alert">{error}</p>}{children}</>;
}
